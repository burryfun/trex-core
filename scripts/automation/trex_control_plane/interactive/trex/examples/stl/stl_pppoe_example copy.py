...
class PPPoETest(object):
    ...
    def inject (self, clients, config_settings):
        ...
        if config_settings.mode == 'BIDIRECT':
            ...
            syn_pkts_split = list(chunks(syn_pkts, CHUNK_SIZE))
            for chunk in syn_pkts_split:
                self.c.push_packets(ports = [0], pkts = chunk, force = True)
            # self.c.push_packets(ports = [0], pkts = syn_pkts, force = True)
            ...
            #! SHIT CODE МОЖЕТ БЫТЬ ОТПРАВЛЯТЬ И ПОЛУЧАТЬ ПАКЕТЫ ДЛЯ 1 КЛИЕНТА? ТАКЖЕ ПОСМОТРЕТЬ scapy.sr1()
            # syn_ack = None
            syn_acks = []
            for client in clients:
                record = client.get_record()
            ...
            for client in successful_clients:
                ...
                for syn_ack in syn_acks:
                    if syn_ack[Ether].type == Ether.type.s2i['PPP_SES']:
            ...
            capture_ack = self.c.start_capture(rx_ports = [0], mode='fixed')
            ack_pkts_split = list(chunks(ack_pkts, CHUNK_SIZE))
            for chunk in ack_pkts_split:
                self.c.push_packets(ports = [0], pkts = chunk, force = True)
            self.c.push_packets(ports = [0], pkts = ack_pkts, force = True)
            self.c.wait_on_traffic(ports = [0])

            # wait for response
            time.sleep(0.1)
            rx_pkts.clear()
            self.c.stop_capture(capture_id = capture_ack['id'], output = rx_pkts)
            ...
            for client in successful_clients:
                ...
                for syn_ack in syn_acks:
                    if syn_ack[Ether].type == Ether.type.s2i['PPP_SES']:
                        #! CHANGE HOST IN GET REQUEST ON SERVER_IP
            ...
            # HTTP 1 packet with response
            # capture_get = self.c.start_capture(rx_ports = [0], mode='fixed')
            # self.c.push_packets(ports = [0], pkts = http_pkts, force = True)
            # self.c.wait_on_traffic(ports = [0])

            # # wait for response
            # time.sleep(0.1)
            # rx_pkts.clear()
            # self.c.stop_capture(capture_id = capture_get['id'], output = rx_pkts)
